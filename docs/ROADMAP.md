...
- Wardrobe upload lifecycle coordination: Firestore draft first, exact private Storage upload second, trusted backend finalisation third, consent-gated future analysis request helper, and non-destructive orphan detection. Helper/rules coverage, Storage trigger handler QA, mobile wardrobe onboarding QA, and upload UI readiness completed on 2026-06-15.
- Private user wardrobe image upload UI to Firebase Storage. Implemented for development-build QA on 2026-06-15 with photo-library selection only, Firestore draft-first creation, exact private Storage path, required metadata, progress, finalisation listening, Wardrobe Home list display, and no AI job creation.
- Wardrobe item management. Initial list display and add-item metadata capture are implemented; edit/delete polish remains later work.
- Weather context integration.
- Occasion-based styling inputs.

Phase 2 status after the 2026-06-15 wardrobe onboarding implementation, Storage trigger QA, upload UI readiness review, and upload UI MVP implementation: onboarding foundation is implemented, setup preferences persist to `wardrobeSetupProfiles/{userId}`, and Wardrobe Home now links to private upload after setup completion. Storage trigger registration and backend finalisation lifecycle are covered in emulators with synthetic uploads and finalized handler payloads. Installed-development-build wardrobe onboarding manual QA passed on 2026-06-15 using the dev-only local QA access harness without creating wardrobe setup data, wardrobe items, Storage files, or AI jobs outside the app flow. The private wardrobe upload UI MVP is implemented, but upload manual QA is pending until the development build is rebuilt with `expo-image-picker`. Production upload enablement must still wait for non-production deployed Storage event verification and explicit approval of cleanup/retention operations.
